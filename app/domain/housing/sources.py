...
여기 실린 아홉 건은 각 `retrieved_at` 날짜에 실제로 열어 본문을 확인한 것만
남긴 것이다. 확인하지 못한 링크는 넣지 않는다 — 근거를 지어내는 것보다 근거가
적은 편이 낫다.

법령 조문 직링크는 `law.go.kr/LSW/lsLinkProc.do?lsNm=<법령명>&joNo=<6자리>&mode=11`
형태를 쓴다. `lsInfoP.do` 계열은 조문 본문을 열지 못했고, 일련번호를 추측해서
맞춘 링크는 다른 법령을 가리킨 적이 있다. 법령명이 URL 에 들어가므로 퍼센트
인코딩된 형태 그대로 저장한다 — 사람이 읽기는 나쁘지만, 실제로 열어 확인한
문자열과 파일에 적힌 문자열이 같아야 한다.
"""
