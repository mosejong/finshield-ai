...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v0.8(72건)이며, v0.7 이
드러낸 결함을 **고치기 전에** 얼려 커밋했다(`9e35a05` → `8243915` → `3bc57f6` →
`304f418` → `5946595` → `202c5c1` → `6034290`, 순서는 `git log`에서 확인된다).
수정 전 f1 0.7143 → 수정 후 1.0000, 미탐 16건 → 0건, 오탐 0건 → 0건,
precision 1.0000 → 1.0000, 오탐률 0.0000 → 0.0000.

**이 만점을 현장 추정치로 읽으면 안 된다.** 72건 전부가 이번 일곱 수정이 건드릴
자리와 건드리면 안 될 자리에 몰려 있어서, 수정이 성공하면 구조적으로 만점이 나온다.
이 표에서 실제로 읽을 것은 accuracy 가 아니라 **precision 과 FPR 이 내려가지
않았다**는 칸이다.

**이번 회차는 값을 치를 자리가 양쪽이었다.** v0.2~v0.5 는 넓히기(값은 정상
문장에서), v0.6 은 좁히기(값은 사기 쪽에서), v0.7 은 올리기(값은 천장에서)였다.
v0.8 은 **넓히기 다섯 + 좁히기 하나**이고, 동결 시점 precision 1.0000 / FPR 0.0000
은 올라갈 자리가 없다. 그래서 정상 36건을 **각 넓히기가 깨질 수 있는 자리에
하나씩** 배치했다 — 정상 권한 위임, 정상 돈 이동과 금지 명령, 진짜 예방 안내문,
정상 기관·입금.

작업 중에 실제로 두 번 깨졌고 **두 번 다 어휘를 좁혀서 피하지 않았다.** 예방
안내문 표지를 옮기자 같은 목록을 쓰는 두 번째 호출 자리가 드러났고, 재전달 어휘를
넓히자 "회비는 나눠 보내지 마시고" 가 걸렸다. 뒤쪽은 어미를 어휘에 붙여 적으면
다음 회차에 다른 어미로 다시 뚫리므로, 금지형을 **자리**의 문제로 보고 닫았다.

`risk_score` 와 `risk_level` 이 서로 다른 것을 재던 것도 이 회차에서 닫혔다.
**등급을 점수에서 다시 계산하지 않았다** — 그러면 여섯 회차의 canonical 판단과 상태
하한이 legacy 가중치 표로 되돌아간다. 대신 점수를 등급이 함의하는 띠까지 올려,
`risk_level: "high"` 와 `risk_score: 0` 이 나란히 실리는 일을 없앴다.

옛 셋 재측정이 회귀 하나를 잡았다(v0.5 `fh-317`). 이진 판정과 등급은 그대로여서
signal coverage 한 칸만 움직였지만, 안전계좌 이체 요구가 `money_mule` 로 불리면
아무것도 받지 않은 사람에게 "받은 돈을 보내지 마세요" 가 나간다. **옛 셋을 다시
재는 것이 이걸 잡으라고 있는 일이다.** v0.7 은 f1 0.8923 → 1.0000 이 됐지만 일곱
수정이 전부 그 셋의 미탐에서 나왔으므로 **독립적인 결과가 아니고** 회귀 확인으로만
읽는다. 개발셋과 v0.2~v0.6 은 한 자리도 움직이지 않았다.

v0.8 에서 고치지 않은 것은 이름과 진단으로 적어 두었다 — 재는 셋 안의 결함을 고치면
그 셋은 성능이 아니라 기억을 재게 된다. **이진 판정이 만점이므로 남은 결함은
이름·행동·등급 쪽에만 있고, 다음 셋은 그쪽을 겨냥해 짜야 한다.**
