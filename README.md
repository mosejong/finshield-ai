...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v0.7(72건)이며, v0.6 이
드러낸 결함을 **고치기 전에** 얼려 커밋했다(`0ce5188` → `af358c4` → `83298e8` →
`8257632`, 순서는 `git log`에서 확인된다). 수정 전 f1 0.7241 → 수정 후 0.8923,
미탐 15건 → 7건, 오탐 1건 → 0건, precision 0.9545 → 1.0000.

**이번 회차는 재는 자를 먼저 고쳤다.** v0.6 이 남긴 결함 중 하나는 등급을
**올리는** 수정인데, `_scenario_policy_accuracy` 가 하한에 대해 `>=` 만 보므로
**모든 메시지에 `high` 를 찍는 엔진이 그 지표에서 1.0 을 받는다.** 넓히기와 좁히기는
각각 값을 치를 자리가 있었는데 올리기만 없었다. 그래서 사례를 쓰기 전에
`expected_max_risk` 와 `risk_ceiling_accuracy` 를 넣었다. 천장은 **정상 문장에만**
선언하고(사기를 높게 매기는 것은 결함이 아니라 신중함이다), 천장을 선언하지 않은
셋에서는 `1.0` 이 아니라 **`None`** 을 돌려준다 — 재지 않은 것을 만점으로 적으면
그것이 곧 지어낸 근거다.

새 지표는 얼리자마자 제 값을 했다. `secrecy_isolation` 이 평범한 비밀 유지 문장
넷에서 이미 켜져 있었고, 그 신호에 사기 유형을 붙이는 순간 네 건이 전부 **사기
판정**이 되는 것이 baseline 에 먼저 기록됐다. 그래서 유형과 어휘를 한 커밋에서
함께 바꿨다.

같은 수정으로 앞선 셋들도 다시 쟀다. **v0.3 이 이진 판정 만점 구간에 들어왔고**
(f1 0.9836 → 1.0000, `fh-138`), **v0.6 은 미탐이 3건 → 1건**(f1 0.9429 → 0.9722,
`fh-446`·`fh-454`)이 됐다. 셋 다 앞선 회차에서 "고치지 않고 이름만 적어 둔"
사례다. 정책 커밋은 **네 셋에서 동시에** 상태 정책 정확도를 올렸다 — 등급이 오른
39건은 일곱 셋 전체에서 **전부 사기**였고, 정상 문장은 한 건도 오르지 않았다.
개발셋은 한 자리도 움직이지 않았다. v0.7 에서 고치지 않은 7건은 이름과 진단으로
적어 두었다 — 재는 셋 안의 결함을 고치면 그 셋은 성능이 아니라 기억을 재게 된다.
