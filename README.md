...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v0.5(72건)이며, v0.4 가
드러낸 결함을 **고치기 전에** 얼려 커밋했다(`8930328` → `107ade4`, 순서는
`git log`에서 확인된다). 수정 전 f1 0.6250 → 수정 후 0.9348, 미탐 19건 → 1건,
오탐률 0.3929 → 0.1786.

**수정 전 오탐률이 v0.4 의 세 배인 것은 셋을 그렇게 만들었기 때문이다.** 이번 수정
네 가지(활용형·반말 요구·예방 안내문·절 범위)는 전부 재현율을 오탐으로 사는
종류라, 72건 중 28건이 그 네 가지를 정확히 깨도록 고른 정상 문장이다. 이 숫자는
현장 추정치가 아니라 **가격표**다.

같은 수정으로 앞선 셋들도 다시 쟀다. v0.4 는 f1 0.9459 → 0.9863(미탐 0건),
v0.2 는 오탐 0건이 됐고, v0.3 과 개발셋은 한 자리도 움직이지 않았다. v0.5 에서
고치지 않은 6건은 이름과 이유로 적어 두었다 — 재는 셋 안의 결함을 고치면 그 셋은
성능이 아니라 기억을 재게 된다.
