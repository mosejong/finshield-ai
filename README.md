...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v0.9(72건)이며, v0.8 이 남긴
결함을 **고치기 전에** 얼려 커밋했다(`c47fa5f` → `ee788d4` → `3928f0b` → `1719966`,
순서는 `git log`에서 확인된다). 이번 회차는 **이진 판정이 한 칸도 움직이지 않는다** —
f1 0.9762, precision 0.9535, recall 1.0000, 오탐률 0.0645 가 수정 전후 같다.
움직인 것은 그 판정에 붙는 **이름·행동·등급**이다.

| 지표 | 수정 전 | 수정 후 |
|---|---|---|
| 필수 행동 coverage | 0.7500 | 0.9625 |
| 금지 행동 회피 | 0.6585 | 0.9268 |
| 상태 정책 정확도 | 0.7222 | 0.9444 |
| 등급 천장 정확도 | 0.9355 | 0.9355 |
| f1 / precision / recall / FPR | 0.9762 / 0.9535 / 1.0000 / 0.0645 | 같음 |

**재는 자를 먼저 고쳤다.** 필수 행동 coverage 만 재면 모든 행동을 다 내보내는 엔진이
만점을 받는다. 그래서 사례에 **금지 행동**(`forbidden_action_codes`)을 적을 수 있게
하고 회피율을 먼저 지표에 넣은 뒤 셋을 얼렸다 — 자를 나중에 고치면 그 자는 결과를
보고 고른 자가 된다. 금지 라벨이 없는 옛 셋에서 이 값은 만점이 아니라 `null` 이다.
**재지 않은 것을 만점으로 적으면 그것이 곧 지어낸 근거다.**

**이번 회차는 갈아 끼우기다.** v0.2~v0.5 는 넓히기(값은 정상 문장에서), v0.6 은
좁히기(값은 사기 쪽에서), v0.7 은 올리기(값은 천장에서), v0.8 은 넓히기 다섯 + 좁히기
하나였다. v0.9 는 **탐지를 더 하지 않는다.** "공식 대표번호로 확인하세요"는 확인할
창구가 있을 때만 실행할 수 있는 조언이다. 이름을 대지 않은 상대에게 같은 말을 하면
사용자는 존재하지 않는 창구를 찾다가 결국 **메시지에 적힌 번호로 건다.**
**틀린 행동은 없는 행동보다 나쁘다.**

그래서 확인 행동을 신호에 고정하지 않고 **그 메시지에 창구가 있는지**로 정한다 —
대출·정책자금, 카드 발급·배송, 투자 권유는 자칭이 없어도 조회 창구를 가리키지만,
재촉·고립 요구·인증정보 요구는 아무 기관도 가리키지 않는다. 여기에 등급 하나를 올리고
(불이익 예고 + 접근수단 요구는 자칭이 없어도 high) 유형 하나를 좁혔다("귀하 명의로
대포통장이 개설되었습니다"는 통장을 **달라는** 말이 아니다).

**옛 셋이 값을 치렀고 그 값을 되돌리지 않았다.** v0.7 은 필수 행동 coverage
0.9583 → 0.8611, v0.6 은 0.9837 → 0.9675, v0.2 는 0.9758 → 0.9636 으로 내려갔다.
잃은 열한 자리가 전부 같은 라벨이다 — 기관도 상품도 없는 메시지에 공식 창구를 요구한
라벨. **v0.8 이 거의 같은 문장에서 그 라벨의 반대를 엔진 결함으로 적어 두었다**
(v0.7 `fh-523` ↔ v0.8 `fh-627`). 뒤에 적힌 판단을 따르되 옛 셋의 라벨은 고치지 않고
그대로 둬서 불일치가 보이게 남긴다. 같은 이유로 v0.8 은 필수 행동 coverage
0.9306 → 1.0000, 상태 정책 정확도 0.9167 → 1.0000 으로 올라갔다. **오탐·미탐은 열
셋 어디에서도 한 건도 움직이지 않았다.**

**동결한 셋 안의 결함은 고치지 않고 적었다.** v0.9 `fh-762`·`fh-764`·`fh-765` 는 지금
정책에서 금지 라벨을 어기고(카드·투자·계좌 안내가 창구를 가리킨다고 본 쪽이 이겼다),
`fh-729` 는 "1시간 안에"가 재촉 어휘에 없어 바닥을 놓치며, `fh-741`·`fh-767` 은 진짜
예방 안내문인데 오탐으로 남는다. 재는 셋 안의 결함을 고치면 그 셋은 성능이 아니라
기억을 재게 된다.
