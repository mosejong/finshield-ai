...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v1.0(70건)이며, v0.9 가 남긴
결함을 **고치기 전에** 얼려 커밋했다(`c6e5ede` → `7604f65` → `c77b782`, 순서는
`git log`에서 확인된다). **새로 상상한 사기 어형이 하나도 없는 첫 회차다** — v0.9 가
"다음 회차 몫" 으로 이름만 적어 둔 결함 다섯이 그대로 다섯 그룹이 됐고, 다섯을 전부
고쳤다.

| 지표 | 수정 전 | 수정 후 |
|---|---|---|
| f1 / precision / recall | 0.8750 / 0.8333 / 0.9211 | 0.9730 / 1.0000 / 0.9474 |
| 오탐률 (FPR) | 0.2188 | 0.0000 |
| 미탐 / 오탐 | 3건 / 7건 | 2건 / 0건 |
| 필수 signal coverage | 0.7188 | 0.9375 |
| 필수 행동 coverage | 0.8788 | 0.9697 |
| 등급 천장 정확도 | 0.7813 | 1.0000 |
| 상태 정책 정확도 | 0.8000 | 0.9571 |

**동결 시점 오탐 일곱 중 다섯이 하나의 어형이었다** — `-(으)면 안 됩니다`. 예방
안내문은 위험한 행동을 말리려고 그 행동을 입에 올려야 하고, 그래서 위험한 낱말이 전부
들어 있다. **하지 말라는 문장이 요구를 세는 자리에서 요구로 세어졌다.**

어형을 하나씩 어휘에서 빼는 대신 **금지 앞자락을 잘라 내고 남은 말로 판단**하게 했다.
어형은 무한하고 어휘는 유한하다. 자르는 것은 절 전체가 아니라 금지까지다 — 사기 문자는
아는 창구를 금지해 놓고 **그 뒤에서** 요구하므로("창구 방문은 하시면 안 되고, 담당자에게
통장 사본을 보내 주셔야 합니다"), 절을 통째로 버리면 오탐 다섯을 지우는 대신 미탐 하나를
새로 만든다. 같은 통찰이 반대편도 닫는다 — **금지된 창구는 창구 안내가 아니다.**

**이번 회차는 전부 넓히기다.** v0.2~v0.5 는 넓히기(값은 정상 문장에서), v0.6 은
좁히기(값은 사기 쪽에서), v0.7 은 올리기(값은 천장에서), v0.8 은 넓히기 다섯 + 좁히기
하나, v0.9 는 갈아 끼우기(값은 옛 셋의 라벨에서)였다. v1.0 은 값이 **한 방향으로만**
치러지므로 셋도 정상을 세게 짰다 — 정상 32건 전부가 등급 천장을 선언하고 전부 `low`
이며, **그중 스물여섯 건이 넓히려는 바로 그 어형을 쓰는 정상 문장**이다. 나머지 네
수정도 같은 규율을 지켰다: 긴급 어휘를 여섯에서 열둘로 늘리면서 **점수를 만드는 표는
일부러 건드리지 않았고**(같은 시간 표현을 평범한 청구서도 쓴다), 띄어 쓴 `대환 대출` 은
무조건 층이 아니라 요구 게이트에만 넣었다.

**오탐이 늘어난 셋은 없고, v0.9 는 여섯 회차 묵은 빚이었다.** v0.9 가 동결 시점에
기록만 하고 고치지 않은 오탐 둘(`fh-741`·`fh-767`)이 바로 이번 어형이라 함께 닫혔고,
그 셋은 이진 판정·등급 천장 만점이 됐다. **결함을 이름으로 적어 두면 나중에 갚을 수
있다.** v0.7 은 양방향으로 움직였고 **잃은 쪽도 적는다** — `fh-520`·`fh-522` 가 라벨이
처음부터 요구한 확인 행동을 내면서 상태 정책이 0.8611 → 0.8889 로 올랐고, 같은 두 건이
`authority_impersonation` 을 유형으로도 내면서 그 유형 precision 이 1.0000 → 0.8462 로
내려갔다. **동결한 셋은 어느 방향으로도 맞춰 쓰지 않는다.**

**얼리는 커밋에 얼린 것을 지키는 테스트가 없으면, 얼린 것이 아니라 적어 둔 것이다.**
동결 커밋이 데이터셋과 baseline 은 넣었는데 통합 테스트에 셋을 등록하지 않았고,
등록하자마자 두 건이 v0.9 의 문장을 그대로 물려받은 것이 걸렸다. 하필 이번 회차가
갚으려던 그 문장이다. **다시 쓰지 않고 뺐다** — 수정이 존재하는 지금 쓰는 문장은 수정을
알고 쓰는 문장이고, 빼기는 결과를 좋게 만들 수 없는 유일한 편집이다(그래서 셋은 70건이고
`fh-839`·`fh-846` 은 결번이다).

**동결한 셋 안의 결함은 고치지 않고 적었다.** v1.0 `fh-803`·`fh-806` 은 미탐으로 남고
(고립 어휘가 "가족에게 말씀하시면 안 됩니다" 를 받지 않고, `-하셔야 합니다` 종결형이
요구 표지에 없다), `fh-825` 는 사기로 잡히지만 신호 이름이 비며, v0.9
`fh-762`·`fh-764`·`fh-765` 의 라벨 재검토는 여전히 열려 있다. 재는 셋 안의 결함을 고치면
그 셋은 성능이 아니라 기억을 재게 된다.
