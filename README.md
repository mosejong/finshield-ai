...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v1.1(79건)이며, 아래 결함을
**고치기 전에** 얼려 커밋했다(`bc5900e` → `902837a`, 순서는 `git log`에서 확인된다).

**이번 회차의 발견은 엔진이 아니라 셋에 있었다.** 열 회차 동안 각 셋에서 사용자
상태(`received_only` 가 아닌 사례)의 수가 이렇게 움직였다.
...
절의 자리 — 그 결함을 가장 깨끗하게 재는 방법은 상태를 고정하는 것이었다. 회차마다 옳은
선택이었고, 열 번 쌓이자 상태 정책 일곱 갈래 중 하나가 표본의 91% 를 차지했다.
**지표가 만점에 가까울 때 물어야 할 것은 "무엇이 남았나" 가 아니라 "이 숫자의 분모가
무엇인가" 다.**

v1.1 은 79건 중 37건이 상태를 갖고 여섯 갈래가 각각 최소 4건이며 그중 열 건이 정상이다.
그러자 상태 정책 정확도가 v1.0 의 0.9571 에서 **0.6582** 로 내려앉았다 — 엔진이
나빠져서가 아니라 재지 않던 것을 재기 시작해서다.

| 지표 | 수정 전 | 수정 후 |
|---|---|---|
| f1 / precision / recall | 0.6875 / 0.9565 / 0.5366 | 0.7647 / 0.9630 / 0.6341 |
| 오탐률 (FPR) | 0.0263 | 0.0263 |
| 미탐 / 오탐 | 19건 / 1건 | 15건 / 1건 |
| 필수 signal coverage | 0.5652 | 0.6522 |
| 필수 행동 coverage | 0.7869 | 0.8852 |
| 등급 천장 정확도 | 1.0000 | 1.0000 |
| 상태 정책 정확도 | 0.6582 | 0.8354 |
| 금지 행동 회피 | 0.9664 | 1.0000 |

**수정 후 f1 0.7647 을 현장 추정치로 읽으면 안 된다.** 앞선 열 셋보다 낮은 이유는 이
셋이 재지 않던 자리를 일부러 세게 짰기 때문이다 — 사기 41건 중 열여섯 건이 이번 회차가
손대지 않기로 한 어휘 공백을 겨냥한다.

**상태가 있는 정상 문장의 천장은 그 상태가 강제하는 바닥이다.** v0.7 이 천장을 도입했을
때는 정상이 전부 `received_only` 라 천장도 전부 `low` 였다. 계좌 정보를 이미 넘긴 사람은
상태 하한 때문에 등급이 `high` 로 강제되고 그것이 옳으므로, 거기에 `low` 를 걸면 그
라벨은 **상태 정책 자체를 결함으로 선언**하는 라벨이 된다. 그 문장이 사기가 아니라는
사실은 **등급이 아니라 유형과 행동으로** 표현되어야 한다.

**상태는 신호를 대신하지 않는다.** 일곱 상태 중 둘만 예방 행동을 냈다 — 계좌 권한·앱
설치·개인정보·송금에는 그 자리가 비어 있었다. **이미 한 번 넘긴 사람이 가장 다시
넘기기 쉬운 사람이다.** 다만 계좌를 알려 주는 일도 앱을 까는 일도 송금하는 일도 대부분은
정상이므로, 위험 신호가 하나라도 켜진 메시지에서만 상태가 예방 행동을 반복한다. 이 네
상태의 정상 5건은 전부 신호가 비어 게이트가 닫힌다.

**사건 어휘가 창구를 가리킨다.** 창구를 기관 자칭 신호에서만 찾고 있었는데, 사기 문자의
절반은 자기가 누구인지 말하지 않고 **무슨 일이 일어났는지만** 말한다. 기관명이 없어도
갈 곳은 명확하다 — 카드 뒷면 번호, 홈택스, 112. 창구가 없다고 판단하면 폴백이 "이전부터
쓰던 연락처" 를 내므로 **미탐 하나가 틀린 행동 하나를 함께 낸다.** `확인`·`절차`·`신청`
은 일부러 뺐다 — 너무 흔하고, 무엇보다 창구를 지목하지 않는다.

**고립을 어휘가 아니라 어형으로 본다.** 목록이 동사에 바로 붙는 금지형만 담고 있어서
`가족에게도 알리지 마세요` 는 잡고 `가족에게 말씀하시면 안 됩니다` 는 통째로 놓쳤다.
어휘를 더 적으면 다음 활용이 또 빠지므로, 고립의 **대상**과 **금지**를 따로 세우고 둘이
한 절 안에 있을 때만 켠다. v1.0 이 요구를 세는 쪽에서 이미 만난 그 어형이다.

**넓히기의 값을 어형이 직접 치렀다.** 고립 어형을 넓히자마자 v0.9 `fh-741`("인증번호는
저희를 포함해 누구에게도 알려 주시면 안 됩니다" — 한국에서 가장 흔한 **정상** 보안
문자)이 오탐이 됐다. 어형이 아니라 글자까지 겹친다. 화자를 예외로 두는 안(`저희를
포함해`)은 버렸다 — 사기 문자가 아무 비용 없이 흉내 낼 수 있다. 가르는 자리는 **금지의
대상**이다: 고립은 이 대화 자체를 말하지 말라고 하고, 보안 안내는 읽는 사람이 이미 쥔
비밀을 말하지 말라고 한다. **인증번호를 말하지 말라는 문장은 인증번호를 요구하지 않는다.**

**얼린 셋은 예측을 반증하기도 한다.** 동결 전에 결함으로 적어 둔 다섯 중 둘은 재 보니
결함이 아니었다 — `-하셔야 합니다` 종결형은 이미 다른 어간이 받고 있었고, 채널 어휘에
스토어 이름을 넣는 것은 겨냥한 것과 **반대 방향**으로 작용한다. 없는 결함을 고치지 않은
것도 이 회차의 결과여서 다섯을 예고했지만 셋만 고쳤다.

**오탐이 늘어난 셋은 없다.** v1.0 이 "다음 회차 몫" 으로 이름을 적어 둔 `fh-803` 이
닫혀 그 셋의 f1 이 0.9730 → 0.9867 이 됐다 — v1.0 이 v0.9 의 여섯 회차 묵은 빚을 갚은
데 이어 **결함을 이름으로 적어 두면 다음 회차가 갚는다**가 두 번 연속 확인됐다.

**두 얼린 셋이 반대 라벨을 달았고, 어느 쪽도 고치지 않았다.** v1.0 `fh-803` 과 v1.1
`fh-901` 은 둘 다 기관을 자칭하지 않고 둘 다 수사를 말하고 둘 다 고립을 요구하는데
`VERIFY_OFFICIAL_CHANNEL` 을 하나는 금지하고 하나는 요구한다. v0.7 `fh-523` vs v0.8
`fh-627` 에서 세운 **"뒤에 적힌 판단을 따른다"** 로 v1.1 을 따랐다 — 수사 사칭 문자를
받은 사람에게 "이전부터 쓰던 연락처로 확인하세요" 는 실행할 수 없는 조언이다. 그 값으로
v1.0 의 금지 행동 회피가 1.0000 → 0.9231 이 됐고 **숫자에 그대로 남겼다.** 테스트가 이
한 건이 기록된 불일치라는 것과 **다른 불일치가 없다**는 것을 함께 못 박는다.

**동결한 셋 안의 결함은 고치지 않고 적었다.** v1.1 의 남은 미탐 15건은 전부 신호 어휘
공백이고(`secrecy_isolation`·`credential`·`familiar_person_claim`·`money_mule` 등),
오탐 1건(`fh-942`)은 **진짜 경찰 통지문**이다 — 요구도 링크도 긴급도 없고 가리키는
창구는 실재한다. 기관 자칭의 무조건 층은 여섯 회차를 버텼지만, **진짜 기관도 문자를
보낸다**는 정상 문장을 열두 셋 중 어디에서도 한 번도 재지 않은 채 버틴 것이다. 재는 셋
안의 결함을 고치면 그 셋은 성능이 아니라 기억을 재게 된다.
