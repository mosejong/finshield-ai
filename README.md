...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v0.4(60건)이며, 투자·리딩방
유인과 지인 사칭 탐지가 **작성되기 전에** 얼려 커밋했다(`5ab2bad` → `6951b2e`,
순서는 `git log`에서 확인된다). 구현 전 f1 0.6552 → 구현 후 0.9459, 미탐 17건 →
1건, **오탐률은 0.1250 그대로**다. 투자 어휘가 정상 은행 문자와 심하게 겹치는데도
(v0.4 정상 24건 중 12건이 일부러 그 어휘를 달고 있다) 새로 깨진 문장이 없었다는
뜻이고, 이 표에서 f1 상승보다 중요한 줄이다. 남은 결함 8건은 고치지 않고 이름으로
