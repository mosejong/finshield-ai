...
**성능 수치는 독립 held-out 쪽을 본다.** 가장 최근은 v0.6(72건)이며, v0.5 가
드러낸 결함을 **고치기 전에** 얼려 커밋했다(`ef60a5e` → `7688dad`, 순서는
`git log`에서 확인된다). 수정 전 f1 0.7838 → 수정 후 0.9429, 오탐 9건 → 1건,
오탐률 0.2500 → 0.0278, precision 0.7632 → 0.9706.

**이번 회차는 수정의 방향이 반대라 값을 치르는 쪽도 반대다.** v0.2~v0.5 가 잰
수정은 전부 넓히는 것이었고 그 값은 정상 문장에서 치러졌다. 이번 세 가지 중 둘은
**좁히는** 것이다(기관 자칭 게이트에 민감 요구 조건을 걸고, 공식 창구를 가리키는
절을 요구에서 뺀다). 좁히기의 값은 **사기 쪽에서** 치러지므로, 이 셋의 안전장치는
부정 사례 수가 아니라 **좁히기가 끊을 수 있는 자리에 놓인 양성 사례 수**이고
통합 테스트가 그것을 센다.

같은 수정으로 앞선 셋들도 다시 쟀다. **v0.5 가 f1 0.9348 → 0.9670, 오탐률
0.1786 → 0.1071, 필수 신호 coverage 1.0000 으로 올라오면서, v0.5 회차에서 "고치지
않고 적어 둔" 3건이 사라졌다.** v0.2 는 신호 coverage 가 0.9016 → 0.9508 로
올랐고, v0.3·v0.4·개발셋은 한 자리도 움직이지 않았다. v0.6 에서 고치지 않은 4건은
이름과 이유로 적어 두었다 — 재는 셋 안의 결함을 고치면 그 셋은 성능이 아니라
기억을 재게 된다.
