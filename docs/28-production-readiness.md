...
둘 중 하나라도 어긋나면 워크플로는 매일 정상적으로 실패하면서 아무도 모르는 상태가 된다. **이 문서를 읽는 시점에 한 번은 직접 확인해야 하는 항목이다.** 2026-08-19 기준 두 조건 모두 켜져 있고, 실패 메일이 실제로 도착하는 것도 확인됐다.

#### 도착하는 것과 알아채는 것은 다른 문제다 (2026-08-19)

메일이 오는지 일부러 실패를 만들어 확인할 계획이었는데, 받은편지함이 이미 답을 갖고 있었다. **8/17–8/18 이틀 동안 "Run failed" 메일이 12통 넘게 와 있었다.** 대부분 다른 저장소(`mosejong.github.io`)의 워크플로 실패고, `finshield-ai` 의 과거 CI 실패도 섞여 있다.

즉 전달 경로는 증명됐지만, 그 증명이 동시에 이 알림의 한계를 드러낸다. **"Run failed" 메일은 이 계정에서 이미 일상이다.** 1년에 한 번 올 인증서 메일 한 통은 그 흐름 안에서 다른 열두 통과 구별되지 않는다. 이 워크플로의 설계는 "메일 도착 = 사람이 안다" 를 전제했는데, 그 전제가 성립하지 않는다.

경보는 **드물어야 신호**다. 두 가지로 나눠 대응한다.

1. **메일 필터** — 발신자가 GitHub 이고 제목에 `Certificate watch` 가 들어간 메일만 별표/라벨로 분리한다. 소음을 줄이는 게 아니라 이 한 통만 다르게 보이게 만드는 것이다. 비용이 가장 싸고 지금 당장 된다.
2. **외부 감시(UptimeRobot 등)** — 아래 표의 이유에 하나가 더 붙는다. 그 채널에는 다른 메시지가 없어서, 뭔가 오면 그 자체가 사건이다. 원래 이번 작업에서 고른 채널이기도 하다.

소음의 근원인 다른 저장소의 실패들을 고치는 것이 근본 대응이지만, 그건 이 저장소의 일이 아니다. 여기서 할 수 있는 것은 **이 알림이 그 소음에 의존하지 않게 만드는 것**뿐이다.
...
- `tests/test_fraud_evaluation.py` — gate 와 **알려진 오답 3건(fg-046/047/049)을 지우지 않고 실제로 고쳤는지**를 고정한다. 2026-08-19 어휘 확장 v0.2 로 셋 다 닫혔고, 테스트는 세 케이스가 골든셋에 그대로 있고 분모가 61 인 상태에서 오류 목록이 비었음을 케이스 단위로 확인한다
...
**같은 날 후속.** 이 결과를 받아 규칙 어휘를 넓혔고(v0.2), 엔진이 골든셋 61건에서 F1 1.000 이 됐다. 뒤집어 적지는 않는다 — **개발셋이 변별력을 잃은 것이지 엔진이 모델보다 낫다는 뜻이 아니다.** 모델은 이 셋을 처음 봤고 엔진은 이 셋으로 다듬어졌다. 이제 이 셋으로는 어느 쪽도 더 잴 수 없으므로, held-out v0.2 가 남은 세 항목 중 하나가 아니라 **다음에 해야 할 단 하나**가 됐다. 격차가 그대로인 칸(행동 coverage, 상태 정책, 근거)은 어휘와 무관하다. 자세한 내용은 `docs/32` "어휘 확장 v0.2 재측정".
