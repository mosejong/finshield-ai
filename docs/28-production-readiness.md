...
### P0-4. 실도메인·DNS·TLS 실환경 검증 — 공개 발급 완료 (2026-08-18), 검증기 미실행
...
**남은 것: `scripts/verify_public_deployment.py` 를 밖에서 돌리는 일.** 위 표는 손으로 확인한 네 가지고, 검증기의 27개 검사는 아직 실행하지 않았다. 외부 TLS 등급 측정도 그대로 남아 있다. 완료 기준을 문장이 아니라 종료코드로 만들어 놓고 그 종료코드를 아직 안 본 상태이므로, 이 항목은 닫지 않는다.
...
7. P0-4 실도메인·DNS·TLS   ← 운영 인증서 발급 완료 (2026-08-18), 검증기 실행 대기
...
7번은 코드·구성·검증기가 모두 준비됐고 `localhost` 예행연습까지 끝났다(`31-public-deployment.md`). **막힌 것은 도메인과 서버 하나뿐이다.** 정해지는 대로 31 문서 3절을 그대로 따라가면 되고, 그 사이에는 8번을 먼저 진행해도 순서가 어긋나지 않는다 — P1-1 알림이 먼저 붙으면 인증서 갱신 감시를 cron 대신 알림으로 시작할 수 있다.
