...
### P1-3. 배포·롤백 절차 — 이미지 파이프라인 동작 확인 (2026-08-18), VM 실배포 미검증
...
- **VM 에 올려 본 적이 없다.** pull 은 개발 머신에서 확인했을 뿐이고, e2-micro 에서 이 스택이 뜨는지는 별개 문제다(메모리 1GB).
...
6. P1-3 배포·롤백 (밖에서 빌드, VM 은 pull)  ← 파이프라인 동작 확인 (2026-08-18), VM 실배포 미검증
