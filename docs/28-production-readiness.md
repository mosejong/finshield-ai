...
### P2-2. LLM 설명 계층 — 계약 경계 착지 (2026-08-18), 프로바이더 미연결

`app/services/llm/` 에 설명 계층의 경계가 들어왔다. 아직 **네트워크로 나가는 코드는 한 줄도 없다.** 실제 프로바이더 구현이 비어 있고, 지금 있는 것은 계약, 프롬프트 고정, PII 최소화, 출력 검증, 실패 처리, 그리고 호출을 기록하는 가짜 프로바이더뿐이다. 프론트의 설명 텍스트는 여전히 mock 계층에서 온다.
...
**남은 것:** 실제 프로바이더 구현(HTTP 호출·타임아웃·비로깅), `evaluation/` 연결로 `llm_only.status` 를 `not_run` 에서 옮기기, prompt injection 골든셋. 사람 이름과 주소는 한국어에서 신뢰할 만하게 잡히지 않아 최소화 계층이 걸러 주지 못한다 — 이 계층은 "덜 보낸다" 이지 "안전하다" 가 아니다.
...
9. P2-1 평가 하네스 bootstrap ← 완료 (2026-08-13) → P2-2 LLM 어댑터 ← 계약 경계 완료 (2026-08-18), 프로바이더 미연결 → held-out·LLM-only·Hybrid
```
