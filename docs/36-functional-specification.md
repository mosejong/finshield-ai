...
### 12.3-b 정확도 (독립 held-out v0.5, 72건)

**이쪽이 성능 수치다.** 아래 셋은 수정 코드가 작성되기 **전에** 얼려 커밋했고
(`8930328`), 수정은 그 다음 커밋(`107ade4`)이다. 순서는 `git log`에서 확인된다.

| 항목 | 수정 전 | 수정 후 |
|---|---:|---:|
| 이진 판정 F1 | 0.6250 | **0.9348** |
| Precision / Recall | 0.6944 / 0.5682 | 0.8958 / **0.9773** |
| FPR | 0.3929 | **0.1786** |
| 필수 신호 coverage | 0.3934 | 0.9836 |
| 필수 행동 coverage | 0.7554 | 0.9856 |
| 상태 정책 정확도 | 0.7361 | 0.9861 |
| 공식 근거 coverage | 1.0000 | 1.0000 |

**수정 전 오탐률 0.3929 는 성능이 아니라 가격표다.** 이번 수정 네 가지(활용형·
반말 요구·예방 안내문·절 범위)는 전부 재현율을 오탐으로 사는 종류라, 72건 중
28건이 그 네 가지를 정확히 깨도록 고른 정상 문장이다. 이 셋의 오탐률을 v0.2·v0.3·
v0.4 와 나란히 두고 읽으면 안 된다 — 구성이 다르다.

### 12.3-c 회귀 (앞선 세 셋과 개발셋)

같은 수정으로 다시 쟀다.

| 셋 | F1 | FPR | 미탐 |
|---|---:|---:|---:|
| 개발셋 v0.1 (61건) | 1.0000 → 1.0000 | 0.0000 | 0 |
| held-out v0.2 (72건) | 0.9545 → **0.9767** | 0.0714 → **0.0000** | 2 |
| held-out v0.3 (60건) | 0.9836 → 0.9836 | 0.0000 | 1 |
| held-out v0.4 (60건) | 0.9459 → **0.9863** | 0.1250 → **0.0417** | 1 → **0** |

**만점이 아니며, v0.5 에서 고치지 않은 6건을 문서에 이름과 이유로 적어
