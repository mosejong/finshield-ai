...
### 12.3-b 정확도 (독립 held-out v0.6, 72건)

**이쪽이 성능 수치다.** 아래 셋은 수정 코드가 작성되기 **전에** 얼려 커밋했고
(`ef60a5e`), 수정은 그 다음 커밋(`7688dad`)이다. 순서는 `git log`에서 확인된다.

| 항목 | 수정 전 | 수정 후 |
|---|---:|---:|
| 이진 판정 F1 | 0.7838 | **0.9429** |
| Precision / Recall | 0.7632 / 0.8056 | **0.9706** / 0.9167 |
| FPR | 0.2500 | **0.0278** |
| 필수 신호 coverage | 0.8246 | 0.9649 |
| 필수 행동 coverage | 0.8943 | 0.9837 |
| 상태 정책 정확도 | 0.8333 | 0.9028 |
| 공식 근거 coverage | 1.0000 | 1.0000 |

**이번 회차는 값을 치르는 쪽이 반대다.** v0.2~v0.5 가 잰 수정은 전부 넓히는
것이었고 그 값은 정상 문장에서 치러졌다. 이번 세 가지 중 둘은 **좁히는** 것이라
그 값이 **사기 쪽에서** 치러진다. 그래서 이 셋은 오탐률이 아니라, 좁히기가 끊을 수
있는 자리에 놓은 **양성** 사례 수로 값을 잰다. 오탐률 0.2500 → 0.0278 만 보고
"세 배 좋아졌다" 고 읽으면 안 된다 — 그 대가로 재현율이 0.8056 → 0.9167 로만
올랐고, `fh-438` 한 건은 좁히기 때문에 놓치는 것으로 기록돼 있다.

상태 정책 정확도 0.9028 이 낮게 남은 것은 탐지가 아니라 **가격표** 문제다. 못 채운
7건 중 6건이 신호는 전부 맞게 켜졌는데 `authority_impersonation` + 넘겨주기 신호
조합이 `HIGH_RISK_SIGNAL_COMBINATIONS` 에 없어 등급이 medium 에 머문 경우다. 탐지
변경과 정책 변경을 한 커밋에 섞으면 귀속이 무너지므로 묶지 않았다.

### 12.3-c 회귀 (앞선 네 셋과 개발셋)

같은 수정으로 다시 쟀다.

| 셋 | F1 | FPR | 미탐 | 필수 신호 coverage |
|---|---:|---:|---:|---:|
| 개발셋 v0.1 (61건) | 1.0000 → 1.0000 | 0.0000 | 0 | 0.9811 |
| held-out v0.2 (72건) | 0.9767 → 0.9767 | 0.0000 | 2 | 0.9016 → **0.9508** |
| held-out v0.3 (60건) | 0.9836 → 0.9836 | 0.0000 | 1 | 0.9811 |
| held-out v0.4 (60건) | 0.9863 → 0.9863 | 0.0417 | 0 | 0.9643 |
| held-out v0.5 (72건) | 0.9348 → **0.9670** | 0.1786 → **0.1071** | 1 → **0** | 0.9836 → **1.0000** |

**v0.5 회차에서 "고치지 않고 적어 둔" 6건 중 3건이 사라졌다**(`fh-308` 미탐,
`fh-360`·`fh-361` 오탐). 그때 적어 둔 진단이 맞았다는 뜻이고, **새 셋을 얼린 뒤에
고쳤으므로 기억을 잰 것이 아니다.**

**만점이 아니며, v0.6 에서 고치지 않은 4건을 문서에 이름과 이유로 적어
두었다**(`docs/32`). 재는 셋 안에서 찾은 결함은 고치지 않는다 — 고치는 순간 그
셋은 성능이 아니라 기억을 재게 된다.
