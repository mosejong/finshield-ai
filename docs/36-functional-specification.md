...
| 호출 정책 | `skip_when_no_evidence` — 근거 블록이 셋 다 비면 부르지 않는다 |

프롬프트를 한 글자라도 고치면 해시가 어긋나 테스트가 깨진다. **모델·프롬프트를
조용히 바꾸는 일이 불가능하다.** 벤치마크 보고서도 같은 해시를 보므로, 다른 지시문에서
잰 설명 품질은 `measured` 가 아니라 `stale` 로 나온다. 호출 정책도 같은 방식으로 본다 —
셋도 지시문도 같은데 **언제 부르는지**가 다르면 그것도 다른 표다. 직전 판 `fraud_explanation_v1`
은 지우지 않고 그대로 남겨 둔다 — 나아졌다는 말은 무엇과 비교했는지가 남아 있을 때만
확인 가능한 주장이다.

**근거가 비면 부르지 않는다.** 프롬프트에 들어가는 세 블록(위험 신호 · 권고 행동 ·
공식 근거)이 전부 비어 있으면 모델을 호출하지 않는다. 그 상태에서는 모델이 무엇을 쓰든
정의상 근거 밖이고, 실측으로 164건 중 61건이 그 모양이었다(`docs/34` 15절). 이 판정에서
사용자가 읽을 문장은 결정론 요약이 이미 채우고 있다.
...
- 근거에 없는 **기관 이름**을 지목하지 않을 것
- **법령·조문을 인용하지 않을 것**
- 근거에도 문자에도 없는 **비율·기한·건수**를 말하지 않을 것
- **결과를 약속하거나 적격성을 단정하지 않을 것**

뒤의 넷은 근거에서 허용 목록을 만들어 대조한다. 연락처 검사가 이미 하던 방식이고
넓힌 것은 **대상**이다. 그리고 **허용 목록의 분모는 검사마다 다르다.**

| 검사 | 허용 목록 | 이유 |
|---|---|---|
| 비율·기한·건수 | 근거 **+ 사용자가 붙여넣은 문자** | 문자가 `24시간 이내` 로 압박하면 설명은 그 기한을 지목해야 한다 |
| 연락처 | 근거만 | 문자에 적힌 번호를 되읽으면 우리 화면이 사기범의 번호에 정당성을 붙여 준다 |

이 비대칭은 빠뜨린 것이 아니라 규칙이며, 테스트가 양쪽 방향을 다 지킨다.

거부 사유는 지표에서 따로 센다 — `rejected_ungrounded_org` ·
`rejected_law_citation` · `rejected_ungrounded_number` ·
`rejected_ungrounded_promise`. 한 문장이 여럿을 어겨도 **처음 걸린 것 하나**만
기록되므로 이 칸이 세는 것은 "어긴 것 전부" 가 아니다.

실측(2026-08-27, 유료 호출 0회): 근거에 없는 기관·법령·기한·비율·상품·절차·
적격성을 지어낸 문장 18건 중 **검사 확대 전에는 18건이 모두 통과**했고, 확대 후
남는 것은 **3건**이다. 같은 회차에서 정상 문장 37건 중 거부는 **0건**이다. 남은
3건은 실재하는 기관에 지어낸 발표를 붙인 문장, 지어낸 상품명, 지어낸 절차로
형태가 정상 문장과 같아 형태 규칙으로는 닿지 않으며, **통과를 요구하는 테스트**로
저장소에 적혀 있다(`docs/34` 17절).

**이것은 사실 검증기가 아니다.** 통과한 문장이 실제로 근거에 붙어 있는지를
판단하는 것은 별도의 판정자가 필요한 일이고, 여기서 하는 것은 판단이 아니라
거름이다.

**그리고 위 37건은 우리가 쓴 문장이지 모델이 쓰는 문장이 아니다.** 2026-09-04 에
공개 URL 에 대표 시연 입력(기관 사칭 + 고립 요구 + 원격 앱 + 송금 요구)을 넣어
보니 주 모델과 대체 모델이 **둘 다** 기관 검사에서 거부됐고, 검사가 지목한 이름은
근거 안에 있는 기관이었다. 원인은 모델이 아니라 정규식이었다 — 기관 꼬리 앞의
어절을 세 개까지 함께 삼킨 뒤 그 덩어리를 통째로 근거에 대조하고 있어서, 이름
앞에 보통 낱말이 오기만 하면 정당한 문장이 죽었다. 37건이 전부 기관 이름을 절 맨
앞이나 쉼표 뒤에 두고 있었기 때문에 셋은 그 질문을 한 적이 없다.

held-out 두 셋에서 옛 규칙과 새 규칙을 같은 호출로 비교한 결과다.
...
고친 뒤 주 모델 성공은 v1.2 51/55, v1.3 46/49 이고 **진짜 거부는 그대로 남는다**
(기관 5 · 법령 1 · 지어낸 연락처 1). 경위와 고친 규칙은 `docs/34` 18절.
...
| 근거 블록이 전부 비어 있음 | **부르지 않는다** — 실패가 아니다 |

**설명이 없는 것보다 틀린 설명이 나쁘다.** 판정은 항상 살아남는다.

마지막 줄은 실패가 아니므로 응답이 달라야 한다. `ExplanationResponse` 가 세 상태를
구분한다.
...
셋을 `explanation === null` 하나로 뭉치면 실측 164건 중 61건에서 화면이 "설명을
불러오지 못했습니다" 라고 말한다 — **아무것도 실패하지 않았는데.**

### 6.4 측정된 지연

지금 코드로 잰 값이다(2026-09-04, held-out v1.2·v1.3 의 유료 호출 104건).
주 모델 p50 7.52초 / p95 9.23초 / 최대 10.27초, 대체 모델은 4건에 약 2.1초다.
판정(밀리초)과 분리돼 있으므로 사용자는 설명을 기다리지 않고 위험 수준과 행동을
먼저 볼 수 있다.

**회차 간 비교로 쓸 수 없다.** 같은 셋 v1.2 가 08-25 에 p50 2.82초, 09-04 에
7.40초였다 — 코드가 아니라 그날의 응답이 흔들린 것이다. 어느 값도 배포 환경
측정이 아니다(13.2).

> 이전 판에 이 자리에 있던 **p50 2.21초 / p95 5.41초는 설명 계층의 값이 아니다.**
> `llm-judge-fraud-v0.1.json` 의 `latency_ms`, 즉 **LLM-only 판정 비교** 61건의
> 지연이다(`docs/32` 3절). 계층을 잘못 붙여 인용하고 있었고 2026-09-04 에 바로잡았다.
...
| 설명 LLM 호출 — 주 모델 (held-out 104건, 2026-09-04) | p50 **7.52초** / p95 **9.23초** |
| 설명 LLM 호출 — 대체 모델 (같은 실행 4건) | 약 **2.1초** (표본이 적다) |

**이 수치는 개발기 측정이며 배포 SLO가 아니다.** 네트워크·TLS·프록시·동시 사용자를
포함하지 않는다. 판정 지연은 미들웨어가 기록하는 **서버 측 `duration_ms`** 이고,
클라이언트 왕복을 재면 httpx 비용이 얹혀 p95 가 5ms 대가 된다. 판정 로직만
따로 재면 p50 0.265ms · p95 0.560ms 이므로 이 표의 대부분은 **엔진이 아니라
HTTP 계층**이다.
...
**동결 시점 = 그 셋을 처음 만난 엔진의 성적**이고, **지금 = 2026-09-04 의 엔진
...
| v0.5 | 72 | 0.6250 | **0.9565** | 4 | 0 | 1.0000 |
...
| v1.0 | 70 | 0.8750 | **1.0000** | 0 | 0 | 0.9844 |
| v1.1 | 79 | 0.6875 | **0.9211** | 0 | 6 | 0.8696 |
| v1.2 | 80 | 0.4384 | **0.9892** | 0 | 1 | 0.9825 |
| v1.3 | 84 | 0.6032 | **1.0000** | 0 | 0 | 1.0000 |

누적 865건: Precision **0.9872** / Recall **0.9789** / F1 **0.9830** /
Accuracy 0.9815 / FPR **0.0153**. 공식 근거 coverage 는 열두 셋 전부 **1.000**.
개발셋을 합한 926건 기준으로는 Precision **0.9882** / Recall **0.9805** /
F1 **0.9843** / FPR **0.0145** 이고, 오탐 6건·미탐 10건은 두 기준이 같다
(개발셋 61건에 오류가 없기 때문이다).
...
- **정직한 held-out 수치는 왼쪽 칸이다.** 처음 보는 셋에서 엔진은 F1 0.44 ~
  ...
  **회귀가 없다는 사실**만 말한다.
- 열두 회차 동안 이전 셋의 F1 이 내려간 적이 없었다. **2026-08-27 에 처음
  값을 치렀다** — v1.2 미탐 `fh-1042` 를 잡으면서 v0.5 에 오탐 `fh-351` 이
  하나 생겼다. 두 문장은 같은 어형이라 규칙으로는 갈리지 않는다. 누적 F1 은
  **0.9819 로 같고**(미탐 12 → 11, 오탐 5 → 6) 바뀐 것은 **어느 쪽으로
  틀릴지**다. 값을 매긴 근거는 `docs/32` 의 2026-08-27 절에 있다.
- **2026-09-03 에 미탐이 하나 더 닫혔다**(11 → 10, 오탐 변화 없음). v1.0
  `fh-806` 이고, 닫은 방법이 이 회차의 내용이다 — `납부` 를 어휘에 넣으면
  926건 점수는 같지만 **셋에 없던 진짜 고지 문자 열 건 중 여섯이 사기가
  된다.** 어휘가 아니라 **계좌 목적지**로 갈랐다. `docs/32` 의 2026-09-03 절.
- 남은 오탐 6건 중 5건은 v0.4·v0.5·v0.6 에서 **그때부터 있던 것**이다.
...
이진 판정과 달리 유형 이름 붙이기는 아직 틀린다. 누적 865건, 열 유형 전부를
약한 순서로 적는다 (2026-09-04 재측정).

| 유형 | F1 | 정밀도 | 재현율 | n |
|---|---:|---:|---:|---:|
| `isolation_coercion` | 0.898 | 0.855 | 0.946 | 56 |
| `acquaintance_impersonation` | 0.907 | 0.919 | 0.895 | 38 |
| `advance_fee_demand` | 0.940 | 0.886 | 1.000 | 39 |
| `authority_impersonation` | 0.946 | 0.925 | 0.969 | 127 |
| `account_access_request` | 0.951 | 0.933 | 0.969 | 129 |
| `smishing_malware` | 0.962 | 0.955 | 0.970 | 66 |
| `investment_scheme` | 0.972 | 0.972 | 0.972 | 36 |
| `money_mule_transfer` | 0.978 | 1.000 | 0.957 | 46 |
| `loan_policy_impersonation` | 0.983 | 1.000 | 0.966 | 29 |
| `card_delivery_impersonation` | 1.000 | 1.000 | 1.000 | 22 |

**가장 약한 자리가 바뀌었다.** 이전 측정에서 최하위였던
`acquaintance_impersonation` 은 0.860 → 0.907 로 올랐고, 지금 가장 낮은 것은
`isolation_coercion` 이다. 정밀도 0.855 / 재현율 0.946 이므로 **덜 잡는 것이
아니라 아닌 것을 부른다** — 비밀 유지를 요구하는 문장이 다른 유형의 사기에도
흔히 붙기 때문이다. 유형 하나를 더 붙이는 실수라 판정 자체는 바뀌지 않지만,
사용자에게 붙는 이름이 하나 더 늘어난다.

`acquaintance_impersonation` 이 여전히 두 번째로 낮은 이유는 4.4절에 있다 —
호칭("엄마야")과 자기지목만으로는 판정하지 않고 **원래 알던 연락처로 확인할 수
없게 만드는 핑계**를 요구하기 때문이다. 이 경계는 다섯 회차째 이월돼 있고,
`docs/10` 은 값을 치를 **정상 짝**을 셋에 먼저 넣어야 한다고 적어 두었다.
...
`pytest -q` **1,292 passed, 2 skipped** (2026-09-05, CI 는 **1,294**).
프론트엔드 `vitest` **151 passed** (22 파일). CI는 매 push마다 테스트, 사기 판정 품질 게이트, 의존성
...
- 설명 문장이 **읽을 만한가** — 2026-08-25 에 설명 계층을 164건에 태워
  계약 준수율은 얻었으나(아래), 사회초년생이 그 220자를 읽고 무엇을 해야
  하는지 알게 되는가는 아직 아무 숫자도 없다
...
| 규칙 엔진 F1 (그 셋으로 고친 뒤) | 0.9892 | 1.0000 |

12.3-b 의 수치는 세 번째 줄, 즉 **고친 뒤** 값이다. 모델에게는 그 열이 없으므로
나란히 놓아야 하는 것은 첫 줄이다. 상태 정책과 필수 행동에서도 동결 시점
엔진보다 모델이 앞선다.

**LLM 줄은 2026-08-25 의 한 번뿐인 측정이고 다시 돌리지 않았다.** 규칙 엔진의
`고친 뒤` 줄만 2026-08-27 엔진으로 갱신했다(v1.2 0.9783 → 0.9892). 이 표에서
날짜가 다른 줄은 그 하나뿐이다.

같은 날 **설명 계층도 같은 두 셋에 처음 태웠다.** 사례 164건, 시도 171회.
설명이 나간 비율은 v1.2 0.9875 / v1.3 1.0000 이고, 안전 필터 차단은 171회 중
0회다. 주 모델 p50 6.4~6.5초 / p95 7.8~8.3초로 14초 예산 안이다.

**그리고 164건 중 6건에서 모델이 없는 연락처를 만들었다.** 결정론 검증기
(`validate_explanation`)가 전부 막았고, 여섯 중 다섯이 이미 돈이나 계좌 권한이
움직인 뒤의 상태다 — 사용자가 지금 전화를 걸 참인 자리다. 탐지만 보면 모델이
엔진을 이기고, 그 모델을 그대로 내보내면 164명 중 6명이 없는 번호를 받는다.
**하이브리드를 고른 이유가 이 두 문장 사이에 있다.** 채점은 요청 경로에서
...
잃고, 오통과는 사기범이 고른 번호를 공식 안내의 얼굴로 내보낸다.

**그래서 같은 날 세 번째 회차에서 근거가 비면 모델을 아예 부르지 않게 했다.** 같은 두
셋에서 **164건 중 61건이 호출 없이 끝났고**(유료 시도 168 → 105), 설명이 나가지 못한
사례가 **양쪽 다 0**, 지어낸 연락처도 **양쪽 다 0** 이다. 검증기는 한 글자도 바뀌지
않았는데 `fh-1007` 이 사라졌다 — 애초에 물어보지 않게 됐기 때문이다. 그 61건에서 화면이
비지 않는 이유는 "왜 위험한지" 블록을 결정론 요약이 먼저 채우기 때문이고, 근거가 없다는
것은 그 요약이 이미 "명시적인 위험 신호가 확인되지 않았습니다" 를 말하고 있다는
뜻이다. 다만 **그 요약만으로 사용자에게 충분한가는 아직 숫자가 없다.** 자세한 것은
`docs/34` 15절.

**위 세 회차는 전부 2026-08-25 이고, 근거 이탈 검사(기관·법령·숫자·약속)는
08-27 에 들어왔다.** 그래서 저장된 표들은 그 검사에 대해 아무 말도 하지
않으면서 최신처럼 보였고, 그 사실이 두 건의 오거부를 여드레 동안 가렸다
(`docs/34` 18·19절). 2026-09-04 에 같은 두 셋을 **지금 코드로** 다시 태웠다.

| | v1.2 | v1.3 |
|---|---:|---:|
| 사례 | 80 | 84 |
| 물어본 건수 | 55 | 49 |
| **설명이 나간 건수** | **54/55** | **49/49** |
| 대체 모델까지 간 사례 | 3 | 1 |
| 유료 호출 | 58 | 50 |
| 지어낸 연락처 | 1 (막힘) | 0 |
| 안전 필터 차단 | 0 | 0 |
| 글자 수 min / p50 / max | 170/238/327 | 138/224/286 |

물어본 104건 중 설명이 나간 것은 **103건**, 사례당 유료 호출 **1.038회**다.
못 나간 하나(`fh-1013`)는 대체 모델이 `medium` 사건에 안심시키는 문장을 내서
막힌 것으로, **맞는 거부**다.

이 실행이 찾아낸 것은 모델의 결함이 아니라 **우리 검사의 결함**이었다. 법령
검사만 허용 목록 없이 무조건 거부하고 있었고, 계좌·인증 접근권을 넘긴 사건에는
엔진 자신이 `국가법령정보센터: 전자금융거래법 제6조 …` 를 공식 근거로 붙여
프롬프트에 넣는다. **모델에게 법령을 읽히고 그것을 되읽었다고 거부한 것이다.**
주 모델과 대체 모델이 같은 사유로 걸려 그 사례만 설명이 아예 없었다 — 모델
둘을 세워 둔 것은 모델의 실패에는 듣지만 **규칙의 결함에는 듣지 않는다.**
법령도 연락처·기관과 같이 근거에 대조하도록 고쳤고(이름과 조문 번호를 따로
본다), **이 변경으로 거부가 늘어나는 입력은 없다.** 위 표는 고친 뒤 실행이다.

남은 거부들은 다시 부르면 통과한다(다섯 사례 25회에서 ok 22 · 거부 3).
`temperature=0.0` 이 결정론이 아니라는 뜻이고, **프로브 숫자 하나를 지표로
읽지 않는다**는 뜻이기도 하다.
...
| 설명 계층 실측 — **지금 코드** (근거 이탈 검사 포함, 2026-09-04) | `evaluation/results/explanation-probe-fraud-holdout-v1.{2,3}-grounding-checks.json` |
| 설명 계층 실측 — 호출 정책 회차 (2026-08-25, 검사 이전) | `evaluation/results/explanation-probe-fraud-holdout-v1.{2,3}-prompt-v2-skip-empty.json` |
| 설명 계층 실측 — 직전 호출 정책 (비교 대상) | `evaluation/results/explanation-probe-fraud-holdout-v1.{2,3}-prompt-v2.json` |
