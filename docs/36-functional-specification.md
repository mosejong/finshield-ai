...
held-out 164건에서 다시 재면 주 모델 p50 2.9~7.1초 / p95 8.9~9.2초로, 개발기 61건보다
느리고 예산 14초 안이다. **두 셋에서 방향이 반대로 움직여**(v1.2 6.5→2.9초, v1.3
6.2→7.1초) 회차 간 비교로 쓸 수 없다. 어느 값도 배포 환경 측정이 아니다(13.2).
...
| 설명 계층 실측 — 배포 구성 | `evaluation/results/explanation-probe-fraud-holdout-v1.{2,3}-prompt-v2-skip-empty.json` |
