...
**양식 원고 · `38-submission-forms.md`** — 주최 측 양식 두 개(기획서 7칸,
기능명세서 5칸)에 그대로 옮길 본문. **사람만 채울 수 있는 칸은
팀명(`FinShield AI`)과 본인 성명 둘**이고 문서 맨 위에 표시돼 있다.
**한글에 붙여넣을 때는 `submission/` 의 `.txt` 두 개를 쓴다** — 마크다운 표·
코드펜스는 HWPX 에서 깨지므로 같은 내용을 개조식 평문으로 옮겨 뒀다. 제출 전 확인 목록도 여기
3절에 있다 (`35` 부록 A 는 낡아서 이쪽으로 옮겼다 — 끝난 일을 최우선으로 적고 있었고,
