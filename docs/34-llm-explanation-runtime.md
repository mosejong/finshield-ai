...
- ~~**안전 필터 차단율 측정.**~~ **셀 수 있게 됐다**(2026-08-24, 12절). 아직 숫자는
  없다 — 계측만 넣었고 유료 실행을 하지 않았으므로 분모가 0 이다. 덧붙여, 원래 이
  항목이 걱정한 "정상 입력이 차단되는" 경우는 `finishReason: SAFETY` 가 아니라
  `promptFeedback.blockReason` 쪽이었다. 둘을 다른 이름으로 센다.
...
- **분모를 채우는 일.** 위 두 항목은 도구를 만든 것이지 답을 얻은 것이 아니다. 유료
  실행을 한 번 돌려 차단율과 시도별 소요시간의 실제 값을 이 문서에 적어야 끝난다.
  그때 2절의 실측 표와 같은 자리에 놓는다.
...
`evaluation/fraud_benchmark.py` 의 `explanation_layer.note` 도 같은 말을 하도록
고쳐 뒀다 — 계측이 있다는 것과 측정했다는 것은 다르다.
