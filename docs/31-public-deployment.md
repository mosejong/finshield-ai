...
## 0. 먼저 읽을 것 — 이 배포에서 조용히 실패하는 네 가지
...
| **중단된 `up` 이 남긴 미기동 컨테이너** | compose 가 도중에 멈추면 그때까지 만든 컨테이너는 **만들어진 채 시작되지 않고** 남는다. 실패 메시지는 멈추게 만든 컨테이너를 가리키므로, 정작 안 뜬 컨테이너의 이름은 어디에도 안 나온다 | `docker compose ps` 의 **목록 길이**를 센다. 무엇이 unhealthy 인지가 아니라 **무엇이 없는지**가 답이다 |
...
네 번째는 2026-09-05 `v0.9.2` 재배포에서 났다. 스크립트가 `dependency failed to start: container finshield-backend-1 is unhealthy` 로 끝났는데, **backend 는 멀쩡했다.** 그 직전에 재시작 중이라 그 순간만 unhealthy 였고, compose 가 전체를 중단한 뒤 혼자 healthy 가 됐다. 중단이 삼킨 것은 방금 `Recreated` 된 **proxy** 였고, 그래서 **443 이 연결을 거부**했다. 공개 URL 이 완전히 죽은 동안 앱 컨테이너 다섯 개는 전부 새 버전으로 healthy 였다.

**진단은 로그가 아니라 목록에서 나왔다.** `docker compose ps` 에 backend·backup·db·retention·web 다섯 개뿐이고 proxy 가 **없었다.** 있는 것은 전부 정상이었으므로 읽을 오류가 없었다 — 없는 줄이 답이었다. 복구는 한 줄이다.
...
**그래서 3-5 를 건너뛰면 안 된다.** 이 사고에서 스크립트의 마지막 안내(밖에서 확인하라)를 따르지 않았다면, 배포가 실패했다는 메시지를 보고 backend 를 고치러 갔을 것이다. 고칠 것이 없는 곳이다.
...
| 2026-09-05 | `v0.9.2` | 3-6 재배포 — **중간에 중단됐고 proxy 가 안 떴다**(§0 네 번째) | 복구 후 밖에서 전수 측정. `/health` `200 ok` 3회 연속·`/health/ready` `ready`. 시나리오 1 `high`·70점·신호 4·유형 3·행동 5·근거 3, 같은 문장 `transferred_money` 로 행동 **9**·근거 **5**(등급·점수·신호는 그대로), 관리비 `low`·0점·신호 0·행동 0·근거 0, 전세 20000/25000/10000 `high`·부채비율 **120%**·`band_is_service_rule: true`·신호 5·할 일 5·근거 5. 설명 계층 `status: ready`(`gemini-3.6-flash`, 240·282자, 2.2·2.4초). 상품 목록 `200` 0.47초(전체 325건 중 100건, 기준월 202608), 상세 `200` 0.56초(취급기관 `신협`), 비교 `200` 0.45초 — 세 응답 모두 `provider: financial_services_commission`. **프로필 저장 경로 10회 연속 10/10**(세션→저장→삭제, 평균 0.48초, 최대 0.55초, 삭제 후 재조회 `404`). 배포된 JS 청크에서 `{financial_services_commission:"금융위원회"}` 와 목록·상세·비교 세 화면의 `공공데이터` 출력 문자열 확인. **계정 전체 삭제 3회 연속 3/3** — `DELETE /api/proxy/auth/account` `204`(0.45—1.16초), 삭제 직후 프로필 재조회 `401`·세션 재조회 `401`·새 세션 재생성 `201`. 배포된 청크에서 확인창 문구·「모든 데이터 삭제 중」·실패 문구·`/api/proxy/auth/account` 모두 확인. 브라우저 검수 — 기관명이 목록·상세·비교 세 화면에 모두 표시(기준월 202608), 「금융 목표 저장→상품 목록」 경로 약 5.3초에 진입, 위험 표현 규칙 준수(문장 우선·full-bleed 빨강·사이렌 없음·tel: 링크·기관명·문서명 표시) — 모두 **PASS**. 2026-09-06 네이티브 확인창 수락 후 삭제까지 사람 눈으로 확인 — **완료 조건 10항목 전부 닫혀 있다.** 남은 미측정은 모바일 폭 실측 하나뿐이고, 이유는 검수 도구에 viewport emulation 이 없기 때문이다(양식에는 로컬 실측값) |
