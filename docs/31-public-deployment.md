...
| 2026-09-05 | `v0.9.2` | 3-6 재배포 — **중간에 중단됐고 proxy 가 안 떴다**(§0 네 번째) | 복구 후 밖에서 전수 측정. `/health` `200 ok` 3회 연속·`/health/ready` `ready`. 시나리오 1 `high`·70점·신호 4·유형 3·행동 5·근거 3, 같은 문장 `transferred_money` 로 행동 **9**·근거 **5**(등급·점수·신호는 그대로), 관리비 `low`·0점·신호 0·행동 0·근거 0, 전세 20000/25000/10000 `high`·부채비율 **120%**·`band_is_service_rule: true`·신호 5·할 일 5·근거 5. 설명 계층 `status: ready`(`gemini-3.6-flash`, 240·282자, 2.2·2.4초). 상품 목록 `200` 0.47초(전체 325건 중 100건, 기준월 202608), 상세 `200` 0.56초(취급기관 `신협`), 비교 `200` 0.45초 — 세 응답 모두 `provider: financial_services_commission`. **프로필 저장 경로 10회 연속 10/10**(세션→저장→삭제, 평균 0.48초, 최대 0.55초, 삭제 후 재조회 `404`). 배포된 JS 청크에서 `{financial_services_commission:"금융위원회"}` 와 목록·상세·비교 세 화면의 `공공데이터` 출력 문자열 확인. **브라우저 전용 칸은 미확인** — 렌더링·`window.confirm` 삭제·tel: 링크·위험 표현 규칙은 재검수 대기 |
