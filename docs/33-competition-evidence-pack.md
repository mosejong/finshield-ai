...
7. 그 비교가 찾아 준 두 오답을 **모델이 아니라 규칙 어휘로** 메웠다(v0.2, 같은 날).
   엔진은 이제 이 61건에서 F1 1.000이다. 다만 이 숫자는 **엔진이 교정에 사용한 셋의
   점수**이므로 일반화의 증거로 제출하지 않으며, 6번의 비교를 뒤집는 근거로도 쓰지
   않는다. 어휘 확장이 골든셋 밖에서 만든 오탐 6건을 별도 문장으로 찾아 되돌린
   과정까지 `docs/32`에 남겼다.
...
- 같은 데이터로 오류를 찾고 규칙을 교정했으므로 독립 held-out 성능이 아니다.
  2026-08-19 어휘 확장 이후 이 셋에서 엔진의 오류가 0건이 됐는데, 이는 **개발셋이
  더 이상 변별하지 못한다**는 뜻으로 읽어야 한다. 만점을 성능 주장으로 제출하지
  않는다.
...
| 어휘 확장 v0.2 해설 | `docs/32-fraud-evaluation-benchmark.md` §어휘 확장 v0.2 재측정 |
| 어휘 확장 구현 | `app/domain/fraud/signals.py` |
| 골든셋 밖 회귀 고정 | `tests/test_fraud_scenario.py` §어휘 확장 v0.2 |
| 시간순 개발·리뷰 | `docs/devlog/2026-08-19/rule-vocabulary-v0.2.md` |

> 이 문서는 **2026-08-19 시점의 v0.1 증거 묶음**이다. 그 뒤 held-out 열두 셋
> 865건으로 측정이 확장됐고, 지금 수치는 `docs/32` 의 2026-09-04 절과
> `docs/35` 7장에 있다. 아래 목록은 당시의 판단을 그대로 두고 완료만 표시한다.

## 다음 증거 단계

1. ~~v0.1을 동결하고 별도 작성자·별도 파일의 held-out v0.2를 만든다.~~
   2026-08-20 완료. 이후 v1.3 까지 **열두 셋 865건**으로 늘었고, 회차마다
   `test: freeze …` 커밋이 `fix:` 커밋보다 앞에 있다.
2. ~~유형별 최소 support와 persona별 분포를 사전에 정한다.~~ 완료.
   사용자 상태 7종을 각 셋이 최소 3건씩 덮고, 유형별 support 는 `docs/32` 의
   2026-09-04 절에 n 과 함께 적혀 있다.
...
4. ~~Hybrid를 구현한 뒤 동일 데이터와 동일 오류 분류로 비교한다.~~ 2026-08-19 완료.
   ...
   2026-08-25 에 held-out v1.2·v1.3 에서 3자 비교를 다시 돌렸다
   (`docs/32` §2026-08-25). **누적 865건 전체로 다시 돌리는 것은 아직 남아
   있다** — 유료 호출 예산이 필요하고, 그래서 `docs/35` 11장의 다음 순위다.
5. 공개 배포 환경에서 TLS·proxy·동시성을 포함한 latency와 error rate를 측정한다.
6. ~~모델이 잡고 규칙이 놓친 `fg-046`·`fg-047`을 v0.2에서 **규칙 어휘로** 메운다.
   모델을 판정 경로에 넣지 않는다.~~ 2026-08-19 완료. `fg-049` 오탐도 함께 닫혔다.
   판정 경로에 모델은 들어가지 않았다.
