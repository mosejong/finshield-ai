...
- [ ] **held-out v0.6** — 아래를 고치기 **전에** 얼려야 한다. v0.5 는 그 수정에 쓰이는 순간 소진된다
- [ ] **`private_channel_invite` 의 투자·매매 맥락 조건** — `단톡방 초대` 는 v0.4 `fh-244`(정상)와 v0.5 `fh-308`(사기)에서 구절이 **똑같다**. 초대 자체가 아니라 무엇을 위한 초대인가가 갈림길이라 맥락 조건이 있어야 어휘에 넣을 수 있다
- [ ] **`authority_impersonation` 의 게이트 모양** — 자칭 *신분*에 요구 조건을 걸고 있는데 신분은 요구의 대상이 아니다. 기관명이 완료 보고절 안에 있으면 신호가 꺼진다(v0.2 `fh-005`). `fh-004`·`fh-012`·`fh-013` 도 같은 자리
- [ ] **절 범위가 가르지 못하는 오탐 3건** — v0.5 `fh-358`·`fh-360`·`fh-361`. 어느 절이 보고인지만 다르고 양성 `fh-323`·`fh-324` 와 표면이 같다
