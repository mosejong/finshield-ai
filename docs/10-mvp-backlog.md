...
- [ ] **held-out v1.0 동결** — 아래 v0.9 결함들을 고치기 **전에**. 이름·행동을 겨냥한 셋이 소진됐으므로 다음 셋은 **설명 문장**이나 **등급의 천장** 쪽으로 옮겨 갈 자리가 있다
- [ ] **`PROHIBITION_ENDINGS` 의 남은 어형** — v0.9 `fh-741`(`주시면 안 됩니다`)·`fh-767`(`…에서만`). **위험한 행동을 제한하려고 그 행동을 입에 올리는 안내문**이 오탐으로 남는다. 동결 시점에 이미 기록했고 이번 세 수정과 무관해 그대로 남았다 — 이 셋 precision 0.9535 의 전부다
- [ ] **긴급 어휘가 여섯 개뿐이다** — v0.9 `fh-729`. `1시간 안에`·`오늘 자정까지`·`내일` 이 전부 빗나간다. 이 한 건이 signal coverage 0.9859 의 전부이고 등급도 그래서 오르지 않았다. 넓히기이므로 값은 정상 쪽에서 치러진다
- [ ] **v0.9 `fh-762`·`fh-764`·`fh-765` 라벨 재검토** — 카드 배송·투자 권유·사설 채널 안내가 조회 창구를 가리킨다고 본 쪽이 이 셋의 금지 라벨을 이겼다. **엔진이 아니라 라벨이 틀렸을 수 있다**
- [ ] **조건부 기관 자칭이 고립 요구만으로는 켜지지 않는다** — 조건부 어휘는 **민감 요구**를 기다리는데 고립 요구가 그 목록에 없다. v0.9 를 짜다가 `fh-712` 에서 발견했고 사례로 만들지 않았으므로 **어느 셋도 이것을 재고 있지 않다**
