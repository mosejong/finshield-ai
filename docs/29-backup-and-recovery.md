...
| | 위치 | 자동화 |
|---|---|---|
| DB 덤프 | VM 의 `./backups` 7세대 | ✅ `backup` 컨테이너 |
| 암호화 키 | 암호 관리자 (호스트 밖) | ❌ 사람이 한 번 |

VM 의 `secrets/` 는 `drwx------`(0700, 소유자 개인)이다. 키를 호스트 밖으로 옮긴 것은 유출 대비이자 **백업**이다 — VM 이 사라지면 덤프 7세대가 전부 열리지 않는 바이트열이 된다.

앞으로 덤프를 GCS 버킷으로 반출할 때(`docs/28` P0-3) **그 버킷에 키를 같이 넣지 않는다.** 넣는 순간 위 표의 두 행이 한 장소가 되고, 이 절의 분리가 무의미해진다. 저장소 쪽 방어는 `.gitignore` 의 `secrets/*` 이고, `tests/test_runtime_secrets.py` 가 `git check-ignore` 로 실제 동작을 확인한다.
